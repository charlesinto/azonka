import {
  ...
  CLEAR_ERROR,
  SUCCESS_RESENDING_PASSCODE,
  EMAIL_VERIFICATION_SUCCESFFUL,
  ERROR_RESENDING_PASSCODE,
  GET_SEC_QUESTIONS,
  LOGOUT_USER,
  EMAIL_FORGOT_PASSWORD_SENT,
  LOGIN_SUCCESS,
  LOGIN_UNSUCCESSFUL,
  PASSWORD_REST_SUCCESSFUL,
  STOP_LOADING,
  FILE_UPLOADED_FALIED,
  FILE_UPLOADED_SUCCESSFULL,
  USER_ROLE_UPDATED_SUCCESSFUL,
  RESET_VERIFY_FORM,
  REDIRECT_SELLER_TO_STORE,
  UNSUCCESSFUL_VERIFICATION,
  SUCCESS_ALERT,
  RESET_VERIFICATION_FORM_STATE,
  UNAUTHORIZED_USER,
  DISPLAY_ERROR,
  FETCH_USER,
...
export const registerUser = (userData) => {
  ...
  const data = {
    ...userData,
    phoneNumber:
      userData.phoneNumber[0] === "0"
        ? userData.phoneNumber.substr(1, userData.phoneNumber.length)
...
export const verifyEmail = (userData) => {
  ...
  return async (dispatch) => {
    try {
      ...
      dispatch({
        type: UNSUCCESSFUL_VERIFICATION,
  ...
  return async (dispatch) => {
    try {
      const response = await axios.post(
        "/api/v1/registration/resend-verification-code",
        {
          emailAddress,
        }
      );
      ...
      if (error.response.status === 401) {
        setTimeout(() => {
          dispatch({
            ...
            payload: "Account deactivated, please contact admininstrator",
          });
        }, 2000);
      }
      if (error.response.data.message)
...
export const resetPasswordWithToken = (userData) => {
  return async (dispatch) => {
    try {
      ...
      dispatch({ type: STOP_LOADING, payload: "" });
      dispatch({
        type: EMAIL_FORGOT_PASSWORD_SENT,
        payload: "Password update successful, please login",
      });
      setTimeout(() => {
      ...
      dispatch({
        type: SUCCESS_ALERT,
        payload: "Password reset successful, please login",
      });
    } catch (error) {
  ...
  return async (dispatch) => {
    try {
      ...
      if (type === "user") {
        ...
        dispatch({
          type: SUCCESS_ALERT,
          payload: "Account upgraded Successfully",
        });
...
export const fileuploadHandler = (file, foldername = "", type = "") => {
  return async (dispatch) => {
    ...
    try {
      ...
      console.log("data", data);
      return dispatch({
        type: FILE_UPLOADED_SUCCESSFULL,
...
export const upgradeToAgent = (validId, validPhoto) => {
  return async (dispatch) => {
    try {
      ...
      dispatch({
        type: SUCCESS_ALERT,
        payload: "Action is successful, your request is been processed",
      });
      dispatch({ type: USER_ROLE_UPDATED_SUCCESSFUL, payload: "" });
...
export const setPin = (userData) => {
  return async (dispatch) => {
    try {
      ...
      dispatch({
        type: FETCH_USER,
        payload: { userData: newUSer, likes, cart },
      });
      dispatch({ type: SUCCESS_ALERT, payload: "Wallet setup successfully" });
...
export const resetAccountPin = (userData) => {
  return async (dispatch) => {
    ...
    try {
      ...
      dispatch({
        type: SUCCESS_ALERT,
        payload: "Action performed successfully",
      });
    } catch (error) {
      if (error.response.data.message)
        return dispatch({

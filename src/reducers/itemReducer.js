...
export default (state = INTIAL_STATE, actions) => {
  switch (actions.type) {
    ...
    case FETCH_HOME_ITEMS_SUCCESSFUL:
      //   console.log("action payload: ", actions.payload);
      return { ...state, homeProducts: actions.payload };
    ...
    case CART_FETCHED_SUCCESSFULLY:
      return {
        ...
        cartData: actions.payload.products,
        redirectToCheckout: false,
      };
    ...
    case ITEM_CHANGE_ACTION:
      return {
        ...handleItemChangeAction(state, actions.payload),
        resetForm: false,
      };
    ...
    case HANDLE_PREFERNCE_CHANGE:
      return {
        ...state,
        [actions.payload.target.name]: actions.payload.target.value,
      };
    case INVALID_ITEM_FORM_DATA:
      return {
        ...state,
        inValidElments: actions.payload.inValidElments,
        validationMessage: actions.payload.validationMessage,
      };
    case ADD_SUB_IMAGES:
...
const editStoreItem = (state, product) => {
  ...
  const filteredSubCategory = state.subCategories.filter(
    (cat) => cat.parentCategory.id === product.category
  );
  ...
  return {
    ...
    subImage1: otherImageUrl1,
    subImage2: otherImageUrl2,
    subImage3: otherImageUrl3,
    subImage4: otherImageUrl4,
...
const handleItemChangeAction = (state, e) => {
  ...
  if (name === "finalPrice") {
    ...
    return {
      ...
      [name]: value.split(",").join(""),
      inValidElments: [...newInvalidElements],
      finalPriceWithComma: numberWithCommas(value.split(",").join("")),
    };
  }
  if (name === "category") {
    const subcatgeories = state.subCategories.filter(
      (category) => parseInt(category.parentCategory.id) === parseInt(value)
    );

...
class Disputes extends Component {
    ...
    exampleCheck2: false,delivery: [], selectedDelivery: [],
    openDisputes: [],
    damaged: false, different: false,
    selectedBox: 'box2',
     exampleCheck1:false, notDelivered: false, orderDelivered: false }
    ...
    getOpenDisputes = async () => {
        try{
            ...
            const response = await Axios.get('/api/v1/user/dispute/get', {headers: {'x-access-token': token}})
    ...
    handleRowClick = (orderId) => {
        const order = this.props.orders.find(item => item.id === parseInt(orderId))
    ...
    handleSubmit = async (e) => {
       ...
       const {message, } = this.state;
        ...
        this.props.initiateRegistration()
        await this.props.createDispute(this.state.selectedDelivery, this.state.message, $fileUpload.get(0).files,
        this.state.damaged, this.state.different)
        await this.setState({
            damaged: false,
            different: false,
            message: '',
    ...
    onDeliverySelect = data => {
        ...
        const index = this.state.selectedDelivery.findIndex(item => item.id === data.id);
        if(index !== -1){
            ...
            components={{
                        Action: (props) => {
                            ...
                            if(props.action.icon === 'view'){
                                return <button title="Resolve" onClick={() => this.resolveDispute(props.data)} className="btn mr-3 btn-lg btn-info">Resolve</button>
                            }
                }
            }}
            columns={[
                { title: 'Dispute Id', field: 'id' },
                {title: 'Date Opened', field:'createdAt', render: (data) => this.convertToDateTime(data.createdAt)},
                {title: 'Status', field: 'status', render: (data) => <span class="badge-soft">{data.status}</span>}
            ]}
            options={{
                headerStyle: {
                    background: '#FA6400',
    ...
    resolveDispute = async (data) => {
        try{
            this.props.initiateRegistration();
            const token = localStorage.getItem('x-access-token');
            await Axios.post(`/api/v1/user/dispute/resolve/${data.id}`, {}, {headers: {'x-access-token': token}})
               ...
               <div className="container-fluid">
                    <div className="row">
                            ...
                            <div class="">
                                ...
                                <div class="card-body">
                                    <ul class="list-group">
                                        <li  onClick={() => this.selectBox('box2')} className={`list-group-item ${this.state.selectedBox === 'box2' ? 'active' : '' } cursor-pointer`}>Orders delivered</li>
                                        <li onClick={() => this.selectBox('box3')} className={`list-group-item ${this.state.selectedBox === 'box3' ? 'active' : '' } cursor-pointer`}>Disputes</li>
                    ...
                    <div className="modal-body">
                        <form onSubmit={this.handleSubmit} >
                            <div class="form-group">
                                ...
                                <textarea value={this.state.message} onChange={this.handleChange} name="message" className="form-control" id="exampleFormControlTextarea1" maxLength="1000" rows="1"></textarea>
                            ...
                            <div class="form-check"style={{display:'flex'}}>
                                <input onChange={this.handleChange} type="checkbox" name="damaged" class="mx-3" style={{display:'block', marginTop:5}} id="exampleCheck1" />
                                <label class="form-check-label" for="damaged" style={{marginTop:'0px !important'}}>Damaged and Defective Item</label>
                            </div>
                            <div class="form-check" style={{display:'flex'}}>
                                <input onChange={this.handleChange} type="checkbox" name="different" class=" mx-3" style={{display:'block', marginTop:5}} id="exampleCheck2" />
                                <label class="" for="different" style={{marginTop:'0px !important'}}>Entirely Different Item</label>
                            </div>
                            <br />
                            <br />
                            ...
                            <MaterialTable
                                components={{
                                    Action: (props) => {
                                        ...
                                        if(props.action.icon === 'check'){
                                            const isChecked = this.state.selectedDelivery.findIndex(item => item.id === props.data.id) !== -1;
                                            return <input onClick={() => this.onDeliverySelect(props.data)} type="checkbox" className=" mx-3" style={{display:'block', marginTop:5}} checked={isChecked}  />
                                        }
                                    }
                                }}
                                columns={[

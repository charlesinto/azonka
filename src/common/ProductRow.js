...
class ProductRow extends Component {
  ...
  componentDidMount() {
    this.setState({
      sum: (this.props.finalPrice / 100) * (this.props.quantity || 1),
      qty: this.props.quantity ? this.props.quantity : 1,
    });
  }
  static getDerivedStateFromProps(nextProps, state) {
    if (state.qty !== nextProps.quantity) {
      return {
        ...state,
        sum: (nextProps.finalPrice / 100) * (nextProps.quantity || 1),
  ...
  handleChange = (e, finalPrice, id) => {
    const newQuantity =
      e.target.value < 0
        ...
        ? 1
        : e.target.value;
    this.props.calSums((finalPrice / 100) * this.state.qty, id, newQuantity);
    this.setState(
      { qty: e.target.value < 0 ? -1 * e.target.value : e.target.value },
      () =>
        this.props.calSums(
          (finalPrice / 100) * this.state.qty,
          id,
          this.state.qty
        )
    );
  ...
  calculateSum = (finalPrice, qty, id) => {
    return this.numberWithCommas((finalPrice / 100) * qty);
  };
        ...
        <div className="row item-row py-3 my-4 bg-white" key={id}>
          <div className=" col-md-6 border-right">
            <div className="d-flex item-name-wrapper">
              <img
                className="item-img rounded-circle"
                src={mainImageUrl}
                alt=".../"
              />
            ...
            <div className="d-flex item-actions hide-mobile">
              ...
              <div>
                <Link
                  to="#"
                  className="btn-move action-order-fonts text-danger"
                >
            ...
            <div className="d-flex justify-content-between mobile-status-price my-4">
              ...
              <p className="mobile-price badge badge-pill badge-primary float-right">
                {" "}
          ...
          <div className="item-qty col-md-2 border-right text-center hide-mobile">
            <div className="qty-div d-flex">
              <div
                ...
                onClick={(e) => this.handleDecreaseQty(e, finalPrice, id)}
                style={{ height: "40px", cursor: "pointer" }}
              >
                <span class="input-group-text qty-sub font-20 text-dark">
                  -
                </span>
              </div>
              <input
              ...
              <div
                ...
                onClick={(e) => this.handleIncreaseQty(e, finalPrice, id)}
                style={{ height: "40px", cursor: "pointer" }}
              >
                <span class="input-group-text qty-add font-20 text-dark">
                  +
                </span>
              </div>
            </div>
          ...
          <div className="item-subtotal col-md-2 border-right text-center hide-mobile">
            ₦ {this.numberWithCommas((finalPrice / 100) * this.state.qty)}
          </div>
            ...
            <div className="container-fluid border-top py-5">
              <div className="d-flex">
                ...
                <div className="d-flex calc-div">
                  <div> ₦ {this.numberWithCommas(finalPrice / 100)}</div>
              ...
              <div className="d-flex my-5 justify-content-end">
                ...
                <span className="mobile-item-subtotal text-primary">
                  {" "}
                  ₦ {(finalPrice / 100) * this.state.qty}
                </span>
              </div>
              <div className="d-flex item-actions justify-content-between">
                <div className="wishlist-mobile-wrap" key={id}>
                  <span>
                    {" "}
                    <i
                      className="fas fa-shopping-bag px-2"
                      onClick={(e) => {
                        this.handleMoveWishList(id);
                      }}
                    ></i>{" "}

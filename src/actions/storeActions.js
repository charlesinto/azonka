  ...
  return async (dispatch) => {
    try {
      const response = await axios.get(
        `/api/v1/seller/store/get-stores/${id}/${pageNumber}`,
        {
          headers: {
  ...
  return async (dispatch) => {
    try {
      const response = await axios.put(
        `/api/v1/seller/store/update/${data.id}`,
        { ...data },
        {
          headers: {
            "x-access-token": localStorage.getItem("x-access-token"),
      ...
      dispatch({
        type: STORE_UPDATED_SUCCESSFULLY,

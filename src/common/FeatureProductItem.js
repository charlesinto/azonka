...
class FeatureProductItem extends Component {
  ...
  componentWillReceiveProps = (props) => {
    if (props.cartItems !== this.props.cartItems) {
      this.setState({
        cartItems: props && props.cartItems,
        cartLength: props.cartItems ? props.cartItems.length : 0,
      });
    }
  };
  ...
  handleDetailModal = async (e) => {
    ...
    let itemDetails = products.filter(
      (data) => data.id === parseInt(e.target.id)
    );
    // console.log(itemDetails)
    await this.props.itemDetailModalAction({
      itemDetails,
      products: this.props.featArray,
    });
        ...
        <div className="product-details">
          ...
          <div className="product-action">
            ...
            <span
              ...
              className="paction add-to-cart-mobile add-cart"
              title="Add to Cart"
              style={{ fontSize: "13px" }}
            ></span>
            <span
              id={id}
              onClick={(e) => this.handleAddCart(e, id)}
              className="paction add-to-cart-desktop add-cart"
              title="Add to Cart"
              style={{ fontSize: "13px" }}
            >
              Add to Cart
...
export default withRouter(
  connect(mapStateToProps, actions)(FeatureProductItem)
);

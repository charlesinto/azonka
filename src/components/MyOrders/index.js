...
class Cart extends Component {
  ...
  getFeaturedCategoriesImages = async () => {
    ...
    const ads = responseAds.data.ads.filter(
      (item) => item.page === Pages.ORDERS_PAGE
    );
    console.log("ads", ads);
          ...
          <div className="col-sm-push-12 col-md-4 col-sm-12">
            <div className="sidebar-item">
              ...
              <form id="coupon-code-form">
                ...
                />
                <button className="button mid secondary">
                  Apply Coupon Code
                </button>
              </form>
            </div>
  ...
  calSums = (sum, productId, qty) => {
    ...
    this.setState(
      {
        quantity: { ...this.state.quantity, [productId]: qty },
      },
      () => {
        const { cartData } = this.state;
  ...
  renderRows = () => {
    ...
    return orders.map((data) => {
      return (
        <OrderProductRow
          calSums={(sum, productId, qty) => this.calSums(sum, productId, qty)}
          calSum={this.calSum}
  ...
  deliveryAddress = () => {
    if (
      this.state.order &&
      this.state.order.addressString &&
      this.state.order.addressString.trim() !== ""
    ) {
              ...
              <div className="col-lg-11 mx-auto  my-5">
                <div className="cart-table-container container">
                  <div className="row item-header">
                    <div className="header-item-orderId col-md-1 ">ORDER</div>
                    <div className="header-item-name col-md-4 ">ITEM</div>
            ...
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">
                  Order Number: {this.state.order && this.state.order.id}
                </h5>
                <button
                  type="button"
                  class="close"
              ...
              <div class="modal-body orders container-fluid">
                <p className="row">
                  <h4 className="col-md-4 col-xs-12">Date Created</h4>{" "}
                  <span className="col-md-8 col-xs-12">
                    {this.state.order &&
                      this.convertToDate(this.state.order.createdAt)}
                  </span>
                </p>
                <p className="row">
                  <h4 className="col-md-4 col-xs-12">Payment Mode</h4>{" "}
                  <span
                    style={{ width: "fit-contents" }}
                    className="col-md-8 col-xs-12 "
                  >
                    <span className="payment-type-wrapper">
                      <span className="payment-type">
                        {this.state.order && this.state.order.paymentType}
                      </span>
                    </span>
                  </span>
                </p>
                <p className="row">
                  <h4 className="col-md-4 col-xs-12">Total Amount Paid</h4>{" "}
                  <span className=" col-md-8 col-xs-12 money">
                    <span>
                      &#8358;{" "}
                      {this.state.order &&
                ...
                <p className="row">
                  <h4 className="col-md-4 col-xs-12">Delivery Status</h4>{" "}
                  <span className=" col-md-8 col-xs-12 text-transform-upper">
                    <span>{this.state.order && this.state.order.status}</span>
                  </span>
                </p>
                <p className="row">
                  <h4 className="col-md-4 col-xs-12">Delivery Address</h4>{" "}
                  <span className=" col-md-8 col-xs-12">
                    <span>{this.state.order && this.deliveryAddress()}</span>
                  </span>
                </p>
                        ...
                        if (props.action.icon === "save") {
                          return (
                            <a
                              ...
                              href={`/store/qrcode-generate/${props.data.id}-${props.data.deliveryCode}`}
